...
def main(main_args):
	...
	## Simulator
	simulator_args = {}
	simulator_args['config'] = '../../maps/D7.cfg' # defines some parameter (available buttons, game variables etc)
	...
	test_prediction_experience_args['memory_capacity'] = 100 # moved from 1 to make test work
	...
	targ_scale_coeffs = np.expand_dims((np.expand_dims(np.array([30.,100.,100.]),1) * np.ones((1,len(target_maker_args['future_steps'])))).flatten(),0)
