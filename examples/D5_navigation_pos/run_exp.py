...
def main(main_args):
	...
	target_maker_args['meas_to_predict'] = [0,1,2,3] # Measurements that we aim to predict
	...
	targ_scale_coeffs = np.expand_dims((np.expand_dims(np.array([30.,100.,100.,100.]),1) * np.ones((1,len(target_maker_args['future_steps'])))).flatten(),0) # addapt dim to meas?
	...
	agent_args['objective_coeffs_meas'] = [1., 0., 0., 0.] # Weights of means for reward
