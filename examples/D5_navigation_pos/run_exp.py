...
def main(main_args):
	...
	target_maker_args['meas_to_predict'] = [0,1] # Measurements that we aim to predict
	...
	#train
	simulator_args['num_simulators'] = 1 # Number of simulations to run in parallel (?)
	...
	targ_scale_coeffs = np.expand_dims((np.expand_dims(np.array([30.,7.5]),1) * np.ones((1,len(target_maker_args['future_steps'])))).flatten(),0) # addapt dim to meas?
	...
	agent_args['objective_coeffs_meas'] = [1., 0.] # Weights of means for reward
