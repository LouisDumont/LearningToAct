...
class FuturePredictorAgentLSTM(Agent):
    ...
    def make_net(self, input_images, input_measurements, input_actions, input_objectives, reuse=False):
        ...
        p_j_lstm = my_ops.lstm_net(p_concat_fc, None, 'p_j_lstm', msra_coeff=0.9)
        # test other method?
            
        p_val_fc = my_ops.fc_net(p_j_lstm, self.fc_val_params, 'p_val_fc', last_linear=True, msra_coeff=0.9) # p_concat_fc
        p_adv_fc = my_ops.fc_net(p_j_lstm, self.fc_adv_params, 'p_adv_fc', last_linear=True, msra_coeff=0.9) # p_concat_fc
